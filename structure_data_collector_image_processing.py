...
from image_manipulation import diff_Image_Masking, draw_Arrows_With_Labels


class Data_Collector_Image_Processing():
    def __init__(
            self, 
        ...
        self.logger = logging.getLogger("Data_Collector_Image_Processing")
    ...
    def write_buffer_to_local(self):
        full_path, name, object_color, image_sample, diff, roberts_sample, roberts_ref, centroid_contour, list_thickness = self.buffer_element_parser(self.path, self.buffer_data_list.pop(0))
        
        draw_org_diff, _, _ = diff_Image_Masking(org_image=image_sample, diff_image=diff, edge_image_sample=roberts_sample, edge_image_ref=roberts_ref)
        ...
        pad_left = 60
        pad_right = 60
        draw_org_diff = cv2.copyMakeBorder(draw_org_diff, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_CONSTANT, value=color)
        centroid_contour = [(x+pad_left, y+pad_top) for x, y in centroid_contour]
        draw_org_diff = draw_Arrows_With_Labels(draw_org_diff, centroid_contour, pad_top, pad_bottom, pad_left, pad_right, labels=list_thickness)
        
        save_image(draw_org_diff, path=full_path, filename=[name], format="png")
    ...
    def buffer_element_parser(path, element):
        ...
        "OK"
        """
        name, id, sector, bg_color, status, object_color, image_sample, diff, roberts_sample, roberts_ref, centroid_contour, list_thickness = element
        ...
        return full_path, name, object_color, image_sample, diff, roberts_sample, roberts_ref, centroid_contour, list_thickness
