...
def save_image(img, path=None, filename=[], format="png"):
    try:
        ...
        logging.exception(
            "Directory creation failed, please report this issue to developer -> {}".format(
                error.__str__()
            )
        )

        #print("Directory creation failed, please report this issue to developer -> {}".format(error.__str__()))
        ...
        if isinstance(source, (list, tuple)):
            ...
            if open_order is None:
                ...
                for i in range(1, length_of_source_list + 1):
                    ...
                    if len(y_title) > 0:
                        row_index = (i - 1) // columns  # Get row index
                        col_index = (i - 1) % columns  # Get column index
                        if col_index == 0 and row_index < len(y_title):
                            plt.ylabel("{}".format(y_title[row_index]))
                    ...
                    plt.xlim(0, source[i - 1].shape[1])
                    plt.ylim(source[i - 1].shape[0], 0)
                    if axis_ticks_range:
                        plt.xticks(range(0, source[i - 1].shape[1], max(1, source[i - 1].shape[1] // axis_ticks_range)))
                        plt.yticks(range(0, source[i - 1].shape[0], max(1, source[i - 1].shape[0] // axis_ticks_range)))
                ...
                if event_click:
                    def onclick(event, is_verbose=True):
                        ...
                        if event.dblclick:
                            ...
                            if is_verbose:
                                logging.info("Chosen: x = %d, y = %d" % (ix, iy))
