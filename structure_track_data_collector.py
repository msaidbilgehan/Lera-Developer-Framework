...
class Track_Data_Collector():
    def __init__(
        ...
        self.logger = logging.getLogger("Track_Data_Collector")
    ...
    def write_buffer_to_local(self):
        ...
        if len(element) == 1 and isinstance(element[0], (list, tuple)) and len(element[0]) > 0:
            # Ensure the data is a list of rows, each with 5 elements
            data = element[0][0]
            if isinstance(data, list) and len(data) > 0 and not isinstance(data[0], (list, tuple)):
                # Single row, wrap in a list
                data = [data]
                
            df = pd.DataFrame(data, columns=self.column_names)
            if not path_control(self.path+"/", is_file=False, is_directory=True)[0]:
                os.makedirs(self.path, exist_ok=True)
            df.to_csv(self.path+'/dataset.csv', mode='a', header=not os.path.exists(self.path+'/dataset.csv'), index=False)
        # else:
            # self.logger.warning("Skipped writing buffer to local: element structure is invalid or empty.")
