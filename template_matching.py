...
class MATCHING:
    ...
    def compute(self, reference_image, sample_image, algorithm=APPLICABLE_ALGORITHMS.BASIC_CROSS_CORRELATION):
        if algorithm.value == 6:
        ...
        elif algorithm.value == 7:
            result = cv2.bitwise_xor(reference_image, sample_image, mask=None)
            ...
            # similarity_match_ratio = cv2.normalize(result_of_method, result_of_method, 0, 1, cv2.NORM_MINMAX)

            # return similarity_match_ratio[0][0]
