...
class Camera_Object():
    ...
    def __init__(
        ...
        # # REMOVE DONT FORGET ## FOR PANO INSPECTION PROJECT ##
        # self.undistortion_camera_matrix = np.load("../laser-printing-control-v3/lenscalibration/195mm/cameraMatrix.npy")
        # self.undistortion_distortion_coefficients = np.load("../laser-printing-control-v3/lenscalibration/195mm/distortionCoefficient.npy")
        # self.undistortion_new_camera_matrix = np.load("../laser-printing-control-v3/lenscalibration/195mm/newcameraMatrix.npy")
    ...
    def quit(self):
        ...
        for thread_id in list(self.__thread_Dict.keys()):
            thread = self.__thread_Dict[thread_id]
    ...
    def connect(self):
        try:
            if self.flag_Camera is CAMERA_FLAGS.BAUMER:
                ...
                # self.instance_Camera.SetImageBufferCount(20) # Close 22.07.2025
                self.instance_Camera.SetImageBufferCount(100)  # Edit 22.07.2025
                # self.instance_Camera.SetImageBufferCycleCount(1)
                # self.instance_Camera.StartStreaming() # Close 22.07.2025
                ...
                # )
                if self.instance_Camera is not None and self.instance_Camera.isOpened():
                    self.logger.info(f"Camera Instance is Successful, The Instance is {self.instance_Camera}")
                    self.is_Camera_Initialized = True
                else:
                    self.logger.error(f"Camera Instance is NOT Successful, The Instance is {self.instance_Camera}")
                    self.is_Camera_Initialized = False
    ...
    def snapshot(self, is_buffer_enabled=True):
        ...
        if self.flag_Camera is CAMERA_FLAGS.BAUMER:
            ...
            '''
            try:
                time_log(id=repr(CAMERA_FLAGS.BAUMER), flag=TIME_FLAGS.START) # time_log(id=repr(CAMERA_FLAGS.BAUMER))

                if self.instance_Camera is not None:

                    local_Last_Snapshot = self.instance_Camera.GetImage().GetNPArray() # Add 21.07.2025

                    # if self.instance_Camera.IsStreaming(): # Close 21.07.2025
                    #     # local_Last_Snapshot = self.instance_Camera.GetImage()
                    #     local_last_snapshot_converted = self.instance_Camera.GetImage().GetNPArray()

                    #     # if not local_Last_Snapshot.IsEmpty():
                    #     #     local_last_snapshot_converted = local_Last_Snapshot.GetNPArray()
                    #     # else:
                    #     #     local_last_snapshot_converted = None
                    #     #     is_Successful = False
                    #     local_Last_Snapshot = -1
                    #     local_Last_Snapshot = local_last_snapshot_converted
                    # else:
                    #     self.logger.debug("BAUMER Camera is NOT Streaming!")
                    #     is_Successful = False

                time_log(id=repr(CAMERA_FLAGS.BAUMER), flag=TIME_FLAGS.END) # time_log(id=repr(CAMERA_FLAGS.BAUMER))
                self.logger.debug(
                ...
                if local_Last_Snapshot is not None:
                    if len(local_Last_Snapshot) != 0:
                        ...
                        self.logger.debug( "snapshot get unsuccessful frame (0 length)!")
                        is_Successful = False
                else:
                    self.logger.debug("snapshot get unsuccessful frame (None)!")
                    is_Successful = False
        ...
        if is_buffer_enabled and is_Successful:
            ...
            ## REMOVE DONT FORGET ## FOR PANO INSPECTION PROJECT ##
            # local_Last_Snapshot = image_Undistortion(
            #     local_Last_Snapshot,
            #     self.undistortion_camera_matrix,
            #     self.undistortion_distortion_coefficients,
            #     self.undistortion_new_camera_matrix
            # )
    ...
    def camera_Releaser(self):
        ...
        elif self.flag_Camera is CAMERA_FLAGS.BAUMER:
            if self.instance_Camera.IsStreaming():
                self.instance_Camera.StopStreaming()
            self.instance_Camera.Disconnect()
            self.logger.debug("camera_Releaser BAUMER Camera is released...")
        else:
    ...
    def basler_Set_Camera_Size(self, resolution):
        if self.instance_Camera is not None:
            if not self.instance_Camera.IsGrabbing():
                self.instance_Camera.Width.SetValue(resolution[0])
                self.instance_Camera.Height.SetValue(resolution[1])
    ...
    def baumer_Camera_Configurations_Protected(self, exposure_time=60000, acquisition_framerate_enable=True, acquisition_framerate=25):
        if self.instance_Camera is not None:
            ...
            try:
                ...
                self.instance_Camera.f.ColorTransformationAuto.SetString("Off")
                # self.instance_Camera.f.ColorCorrection.Set("OptimizedMatrixFor9500K")
                # self.instance_Camera.f.CalibrationMatrixColorSelector.SetString("OptimizedMatrixFor6500K")
                self.instance_Camera.f.ColorTransformationFactoryListSelector.SetString("OptimizedMatrixFor5000K")
    ...
    def basler_Camera_Configurations_Protected(self, exposure_time=60000, acquisition_framerate_enable=True, acquisition_framerate=25):
        if self.instance_Camera is not None:
            try:
                ...
                # self.instance_Camera.LightSourcePreset.Value = "Daylight5000K"
                self.set_Exposure_Time(exposure_time)
            ...
            self.logger.info(
                "BAUMER Camera: Model Name is '{}', Exposure Time is '{}'".format(
