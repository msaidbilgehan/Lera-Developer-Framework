...
class Camera_Object():
    ...
    def __init__(
        ...
        """
        for id in range(index_device):
            self.buffer_Stream[id] = Structure_Buffer(max_limit=max_buffer_limit)
            self.buffer_Camera[id] = Structure_Buffer(max_limit=max_buffer_limit)
        """
        
        self.buffer_Stream = dict()
        for id in range(index_device):
            self.buffer_Stream['cam'+str(id)] = Structure_Buffer(max_limit=max_buffer_limit)
        self.buffer_Camera = Structure_Buffer(max_limit=index_device)
        
        # self.buffer_Stream = [Structure_Buffer(max_limit=max_buffer_limit) for _ in range(index_device)]
        # self.buffer_Camera = [Structure_Buffer(max_limit=max_buffer_limit) for _ in range(index_device)]

        self.instance_Camera_Array = None
    ...
    def initialize(
        ...
        if self.flag_Camera is CAMERA_FLAGS.BAUMER:
            ...
            self.api_Basler_Camera_Create_Instance(camera_count=index_device) # init self.instance_Camera_Array
        ...
        elif self.flag_Camera is CAMERA_FLAGS.IP:
            self.api_IP_Camera_Create_Instance(camera_count=index_device, list_rtsp_ip=extra_params)
            pass

        # is_Camera_Initialized = (False or True) is done in initialize function
        # Do Not handle not initialized scenario!
        if self.connect() is True: # assign self.buffer_Camera[0, 1, ..]
            if auto_configure:
                if self.flag_Camera is CAMERA_FLAGS.BAUMER:
                    ...
                    # TODO: UNDER CONSTRUCTION!
                    # self.cv2_Set_Camera_Exposure(exposure_time)
                    pass
    ...
    def auto_detect(self, trigger_quit=None, trigger_pause=None, delay=0.1):
        self.logger.info("auto_detect - Still at development...")
        """
    ...
    def quit(self):
        ...
        for id in range(len(self.buffer_Stream)):
            self.logger.info("quit Buffer Stream {} cleaned. {} number of buffer element removed.".format(id, len(self.buffer_Clear(buffer=self.buffer_Stream['cam'+str(id)]))))
        self.logger.info("quit Buffer Camera {} cleaned. {} number of buffer element removed.".format(id, len(self.buffer_Clear(buffer=self.buffer_Camera))))
        
        # if len(self.thread_Dict.items()) > 0:
            # for thread_id, thread in self.thread_Dict.items():
            
        for thread_id in list(self.thread_Dict.keys()):
    ...
    def undistort(self):
        self.logger.info("undistort - Still at development...")
    ...
    def connect(self):
        try:
            if self.flag_Camera is CAMERA_FLAGS.BAUMER:
                ...
                if self.instance_Camera_Array is not None:
                    
                    for id in range(len(self.buffer_Camera)):
                        if not self.buffer_Camera.get(id).IsGrabbing():
                            self.buffer_Camera.get(id).StartGrabbing(pylon.GrabStrategy_LatestImageOnly)
                        
                    self.logger.info(
                        "connect-Camera Instance is Successful, The Instance is {}".format(
                            self.instance_Camera_Array
                        )
                    )
                ...
                for id in range(len(self.buffer_Camera)):
                    if self.buffer_Camera.get(id).isOpened():
                        self.logger.info(
                            "connect-Camera Instance is Successful, The Instance is {}".format(
                                self.buffer_Camera.get(id)
                            )
                        )
                    else:
                        self.logger.warning(
                            "connect-Camera Instance is not Successful, The Instance is {}".format(
                            self.buffer_Camera.get(id)
                            )
                        )
    ...
    def snapshot(self, cam_id=0, is_buffer_enabled=True):
        is_Successful = False
        ...
        if self.flag_Camera is CAMERA_FLAGS.BAUMER:
            ...
            try:
                ...
                if self.instance_Camera_Array is not None:
                    ...
                    if self.buffer_Camera.get(cam_id).IsGrabbing():
                        
                        local_Last_Snapshot = self.buffer_Camera.get(cam_id).RetrieveResult(
                        ...
                        if local_Last_Snapshot.GrabSucceeded():
                            ...
                            cam_id = self.buffer_Camera.get(cam_id).GetCameraContext()
            ...
            try:
                time_log(id=repr(CAMERA_FLAGS.IP), flag=TIME_FLAGS.START)
                if self.instance_Camera_Array is not None:
                    
                    if self.buffer_Camera.get(cam_id).isOpened():
                        
                        ret, local_Last_Snapshot = self.buffer_Camera.get(cam_id).read()
                        if ret:
                            local_last_snapshot_converted = local_Last_Snapshot
                        else:
                            self.logger.debug("IP Camera Grab is NOT succeed!")
                            local_last_snapshot_converted = None
                            is_Successful = False
                        local_Last_Snapshot = local_last_snapshot_converted
                    else:
                        self.logger.debug("IP Camera is NOT Grabbing!")
                        is_Successful = False

                time_log(id=repr(CAMERA_FLAGS.IP), flag=TIME_FLAGS.END)
                self.logger.debug("{} Time passed: {:.3f} ms".format(
                        repr(CAMERA_FLAGS.IP),
                        time_list[repr(CAMERA_FLAGS.IP)][TIME_FLAGS.PASSED]
                    )
                )
                """
                # debug:
        ...
        if is_buffer_enabled and is_Successful:
            ## REMOVE DONT FORGET ## FOR LANDIRENZO PROJECT ## 
            # local_Last_Snapshot = image_Rotate(local_Last_Snapshot)
            # local_Last_Snapshot = image_Flip(local_Last_Snapshot, task='verticaly')
            
            self.buffer_Stream['cam'+str(cam_id)].append(local_Last_Snapshot)
        ...
        """
        stdo(1, "[{}]: {} snapshot-Calculated FPS: {:.2f}".format(
                cam_id,
                repr(CAMERA_FLAGS.BASLER),
                1000 / time_list[self.name][TIME_FLAGS.PASSED]
            )
        )
        """
    ...
    def stream_Start_Thread(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, cam_id=0):
        if self.get_Is_Object_Initialized():
            self.thread_Dict["stream_Start_Thread"] = Thread_Object(
            ...
            self.thread_Dict["stream_Start_Thread"].init(
                params=[
                    trigger_pause,
                    trigger_quit,
                    number_of_snapshot,
            ...
            self.thread_Dict["stream_Start_Thread"].start()
            
            return self.thread_Dict["stream_Start_Thread"]
        else:
            return None
        
    def stream_Start_Thread_2(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, cam_id=1):
        if self.get_Is_Object_Initialized():
            self.thread_Dict["stream_Start_Thread_2"] = Thread_Object(
                name="Camera_Object.stream_Start_Thread_2",
                delay=0.0001,
                logger_level=self.logger.getEffectiveLevel(),
                set_Daemon=True,
                run_number=1,
            ...
            self.thread_Dict["stream_Start_Thread_2"].init(
                params=[
                    trigger_pause,
                    trigger_quit,
                    number_of_snapshot,
                ...
                task=self.stream_Start_Cam2
            )
            self.thread_Dict["stream_Start_Thread_2"].start()
            
            return self.thread_Dict["stream_Start_Thread_2"]
        else:
            return None
        
    def stream_Start_Multiple_Thread(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, cam_id=0):
        if self.get_Is_Object_Initialized():
            self.thread_Dict["stream_Start_Multiple_Thread_"+str(cam_id)] = Thread_Object(
                name="Camera_Object.stream_Start_Multiple_Thread_"+str(cam_id),
                delay=0.0001,
                logger_level=self.logger.getEffectiveLevel(),
                set_Daemon=True,
                run_number=1,
            ...
            self.thread_Dict["stream_Start_Multiple_Thread_"+str(cam_id)].init(
                params=[
                    trigger_pause,
                    trigger_quit,
                    number_of_snapshot,
                ...
                task=self.stream_Multiple_Start
            )
            self.thread_Dict["stream_Start_Multiple_Thread_"+str(cam_id)].start()
            
            return self.thread_Dict["stream_Start_Multiple_Thread_"+str(cam_id)]
        else:
            return None
    ...
    def stream_And_Save_Start_Thread(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, save_path="video.avi", fps=24):
        if self.get_Is_Object_Initialized():
            self.thread_Dict["stream_And_Save_Start_Thread"] = Thread_Object(
            ...
            self.thread_Dict["stream_And_Save_Start_Thread"].init(
                params=[
                    trigger_pause,
                    trigger_quit,
                    number_of_snapshot,
            ...
            self.thread_Dict["stream_And_Save_Start_Thread"].start()

            return self.thread_Dict["stream_And_Save_Start_Thread"]
        else:
            return None
    ...
    def save_Video_From_Buffer_Thread(self, path="", name="capture", codec="MJPG", extension="avi", fps=24, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001):
        if self.get_Is_Object_Initialized():
            self.thread_Dict["save_Video_Start_Thread"] = Thread_Object(
            ...
            self.thread_Dict["save_Video_Start_Thread"].init(
                params=[
                    path,
                    name, 
                    codec, 
            ...
            self.thread_Dict["save_Video_Start_Thread"].start()

            return self.thread_Dict["save_Video_Start_Thread"]
        else:
            return None
    ...
    def stream_Start_And_Save(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, save_path="", fps=24):
        ...
        while number_of_snapshot != 0:
            ...
            if trigger_quit is not None and not self.__instance_Exit_Statement:
                ...
                break
            # stdo(1, "Buffer: {}".format(len(self.buffer_Stream)))
            sleep(delay)
    ...
    def stream_Start(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, cam_id=0):
        if trigger_pause is None or not callable(trigger_pause):
            trigger_pause = self.trigger_pause
        ...
        while number_of_snapshot != 0:
            if trigger_pause():
                ...
                # (self.instance_Camera is None) Means we got quit trigger from camera recovery function, thus quit from thread.
                if self.instance_Camera_Array is None:
                    self.logger.debug(
                        "stream_Start get instance_Camera as None caused by active quit trigger. Quitting..."
                    )
                ...
                is_Successful, snapshot = self.snapshot(cam_id=cam_id, is_buffer_enabled=True)
    ...
    def stream_Start_Cam2(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, cam_id=1):
        if trigger_pause is None or not callable(trigger_pause):
                trigger_pause = self.trigger_pause
    ...
    def stream_Multiple_Start(self, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, delay=0.001, trigger_before=None, trigger_after=None, cam_id=0):
        if trigger_pause is None or not callable(trigger_pause):
            trigger_pause = self.trigger_pause

        pause_Delay = 1
        while number_of_snapshot != 0:
            if pause_Delay > 0: #trigger_pause():
                if not self.is_Camera_Active():
                    self.logger.debug(
                        "stream_Multiple_Start get Camera NOT Active. 'camera_Recovery' calling...")
                    self.camera_Recovery(
                        trigger_quit=trigger_quit, 
                        trigger_pause=trigger_pause
                    )
                ...
                if self.instance_Camera_Array is None:
                    self.logger.debug(
                        "stream_Multiple_Start get instance_Camera as None caused by active quit trigger. Quitting..."
                    )
                    return 0
    ...
    If not then it will wait for a new frame to arrive by calling self.buffer_Stream.get() which blocks until a new frame arrives or until timeout of 1 second occurs (if no frames have arrived).
    - generated by stenography autopilot 
    '''
    ...
    def stream_Connector(self, connection, trigger_pause=None, trigger_quit=None, number_of_snapshot=-1, auto_pop=True, pass_broken=True, delay=0.001):
        ...
        while number_of_snapshot != 0 and trigger_pause():
            if len(self.buffer_Stream) > 0:
                # call ui.set_display function that given frame-buffer as parameter

                frame = self.buffer_Stream.pop() if auto_pop else self.buffer_Stream.get_Last()
        ...
        if len(self.buffer_Stream) > 0:
            ...
            if pass_broken:
                if frame is not None:
                    ...
                    self.logger.debug(
                        "stream_Connector Broken None frame passing..."
                    )
            else:
                return frame
        else:
    ...
    def is_Camera_Active(self):
        if self.flag_Camera is CAMERA_FLAGS.BAUMER:
            ...
            self.camera_Last_Statement = False if self.instance_Camera_Array is None else True
            ...
            if self.camera_Last_Statement:
                self.logger.debug(
                    "is_Camera_Active get True from IP Camera Control. Returning True...")
                return self.camera_Last_Statement
            else:
                self.logger.debug(
                    "is_Camera_Active get True from IP Camera Control. Returning False...")
                return self.camera_Last_Statement
    ...
    def camera_Releaser(self):
        ...
        if self.instance_Camera_Array is None:
            ...
            for id in range(len(self.buffer_Camera)):
                if self.buffer_Camera.get(id).IsGrabbing():
                    self.buffer_Camera.get(id).StopGrabbing()
                self.buffer_Camera.get(id).Close()
            self.instance_Camera_Array.Close()
            ...
            for id in range(len(self.buffer_Camera)):
                if self.buffer_Camera.get(id).isOpened():
                    self.buffer_Camera.get(id).release()
            self.instance_Camera_Array.release()
            self.logger.debug("camera_Releaser IP Camera is released...")
    ...
    def set_Exposure_Time(self, cam=None, exposure_time=20000):
        self.exposure_time = exposure_time
        if self.flag_Camera is CAMERA_FLAGS.BAUMER:
            self.baumer_Set_Camera_Exposure(exposure_time)
        elif self.flag_Camera is CAMERA_FLAGS.BASLER:
            self.basler_Set_Camera_Exposure(cam, exposure_time)
        elif self.flag_Camera is CAMERA_FLAGS.CV2:
            self.cv2_Set_Camera_Exposure(exposure_time)
    
    def get_Exposure_Time(self, cam=None):
        if self.flag_Camera is CAMERA_FLAGS.BAUMER:
            pass
        elif self.flag_Camera is CAMERA_FLAGS.BASLER:
            self.basler_Get_Camera_Exposure(cam)
        elif self.flag_Camera is CAMERA_FLAGS.CV2:
            pass
    ...
    def api_Set_Camera_Size(self, resolution=(1280,720)):
        ...
        if self.flag_Camera is CAMERA_FLAGS.BAUMER:
            ...
            self.basler_Set_Camera_Size(self.resolution)
        elif self.flag_Camera is CAMERA_FLAGS.CV2:
    ...
    def basler_Set_Camera_Size(self, resolution):
        if self.instance_Camera_Array is not None:
            for id in range(len(self.buffer_Camera)):
                if not self.buffer_Camera.get(id).IsGrabbing():
                    self.buffer_Camera.get(id).Width.SetValue(resolution[0]) 
                    self.buffer_Camera.get(id).Height.SetValue(resolution[1])
    ...
    def api_Basler_Camera_Create_Instance(self, camera_count=2):
        global pylon
        try:
            from pypylon import pylon
            
            self.instance_Camera_Array = pylon.InstantCameraArray(
                min(camera_count, len(pylon.TlFactory.GetInstance().EnumerateDevices()))
            )
            
            tl_factory = pylon.TlFactory.GetInstance()
            devices = tl_factory.EnumerateDevices()
            
            for id, cam in enumerate(self.instance_Camera_Array):
                cam.Attach(tl_factory.CreateDevice(devices[id]))
                self.buffer_Camera.append(cam)
                
                self.logger.info(
                    "api_Basler_Camera_Create_Instance: [{}] Model Name is '{}'".format(
                        id,
    ...
    def api_IP_Camera_Create_Instance(self, camera_count=3, list_rtsp_ip=[]):
        try:
            
            self.instance_Camera_Array = cv2.VideoCapture(list_rtsp_ip[0]) # TODO: Check if we need to create instance for each IP or just one
            
            for id, ip in enumerate(list_rtsp_ip):
                
                self.buffer_Camera.append(cv2.VideoCapture(ip))
                self.logger.info(
                    "api_IP_Camera_Create_Instance: [{}] RTSP IP is '{}'".format(
                        id,
                        ip
                    )
                )
        ...
        if self.instance_Camera_Array is not None:
            try:
                ...
                for id in range(len(self.buffer_Camera)):
                    ...
                    self.buffer_Camera.get(id).Open()
                    self.buffer_Camera.get(id).LightSourcePreset.Value = "Daylight6500K"
                    self.set_Exposure_Time(cam=self.buffer_Camera.get(id), exposure_time=exposure_time)
                    
                    if not self.buffer_Camera.get(id).IsGrabbing():
                        self.buffer_Camera.get(id).StartGrabbing(pylon.GrabStrategy_LatestImageOnly)
                    
                    self.logger.info(
                        "BASLER Camera: Model Name:{}, Exposure Time:{}, FPS:{:.2f}".format(
                            self.buffer_Camera.get(id).GetDeviceInfo().GetModelName(),
                            self.buffer_Camera.get(id).ExposureTime.GetValue(),
                            self.buffer_Camera.get(id).ResultingFrameRate.Value
                        )
                    )
                
            except Exception as error:
                self.logger.error("api_Basler_Camera_Configurations-Exception: {}".format(error))
                self.is_Camera_Initialized = False
    ...
    def basler_Set_Camera_Exposure(self, cam, value):
        if self.instance_Camera_Array is not None:
            cam.ExposureTime.SetValue(value)
            
    def basler_Get_Camera_Exposure(self, cam):
        if self.instance_Camera_Array is not None:
    ...
    def set_Buffer_Size(self, custom_buffer_id=0, size=1):
        self.buffer_Stream['cam'+str(custom_buffer_id)].update_Buffer_Size(size)
    
    def get_Buffer_Size(self, custom_buffer_id=0):
        return len(self.buffer_Stream['cam'+str(custom_buffer_id)])
    ...
    def get_Buffered_Image(self, custom_buffer_id=0, index=0):
        return self.buffer_Stream['cam'+str(custom_buffer_id)].get(index) if len(self.buffer_Stream['cam'+str(custom_buffer_id)]) > index else None
    ...
    def buffer_Clear(self, buffer=None):
    ...
    def buffer_Connector(self, custom_buffer, custom_buffer_id=0):
        if type(custom_buffer) is Structure_Buffer:
            self.buffer_Stream["cam"+str(custom_buffer_id)] = custom_buffer
    ...
    def get_information(self, filter_keys=None, rename=None, return_only_text=True, return_only_dict=True, text_format="{}:{}", ends="\n"):
        ...
        dict_information["buffer_max"] = self.buffer_Stream.max_limit
        dict_information["buffer_len"] = len(self.buffer_Stream)
